...
def ACME_summary_plot(plot_df, meta):
    ...
    title = 'Local AcME: observation ID ' + str(meta['index']) + '. Predicted: ' + str(round(meta['x'], 3)) if meta[
        'local'] else 'AcME Global Importance'
    ...
    # set hover template
    fig.update_traces(
        hovertemplate='Feature: <b>%{y}</b><br><br>' + label_x + ': %{x}<br>Original value: %{customdata[0]}<br>Quantile: %{marker.color}')
