...
from ACME.ACME_anomaly_detection import build_anomaly_detection_feature_exploration_table, \
    computeAnomalyDetectionImportance
...
class ACME():
    ...
    def explain(self, dataframe, robust=False, label_class=None):
        ...
        if self._task in ['r', 'reg', 'regression'] or self._task in ['ad', 'anomaly detection']:
            feature_table, importance_table, baseline_pred, baseline, perc_functions = computeACME(model=self._model,
                                                                                                   task=self._task,
        ...
        if self._task in ['c', 'class', 'classification']:
            ...
            for lab in label_list:
                ...
                feature_table, importance_table, baseline_pred, baseline, perc_functions = computeACME(
                    model=self._model, task=self._task,
                ...
                if len(label_list) > 1:
                    importance_table.rename(columns={'importance': 'importance_class_' + str(label_class[lab])},
                                            inplace=True)
        ...
        self._global_explain = {'meta': feature_table.copy(),
                                'feature_importance': importance_table.sort_values('importance',
    ...
    def explain_local(self, series, label_class=None):
        ...
        if len(self._global_explain.keys()) == 0:
            raise InterruptedError(
                "You must first fit the acme explaination model with the 'explain' command on the same data used to train the model")
        ...
        if self._task in ['c', 'class', 'classification']:
            ...
            if label_class is not None:
                ...
                print(
                    'WARNING: in local interpretation, the label_class must be specified and not None. To default it\'s setted to class:' + str(
                        class_map[0]))
        ...
        self._local_explain = {'meta': local_table.copy(),
                               'feature_importance': local_importance_table.sort_values('importance',
                                                                                        ascending=False).copy(),
    ...
    def summary_plot(self, local=False, weights={}, n_features: int = None):
        ...
        # generate the quantile/feature/effect plot
            ...
            plot_df = ordered_blocks.sort_values('importance', ascending=True).reset_index().rename(
                columns={'index': 'feature'}).copy()
