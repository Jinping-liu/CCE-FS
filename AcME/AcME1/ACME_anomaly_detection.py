...
def computeAnomalyDetectionImportance(local_table, weights={}):
    ...
    for feature in local_table.index.unique():
        ...
        tmp['state_pred_change'] = (
                    np.sign(tmp['baseline_prediction'] * 2 - 1) != np.sign(tmp['predict'] * 2 - 1)).astype(int)
    ...
    importance_df['importance'] = importance_df['delta'] * weights['delta'] + importance_df['ratio'] * weights[
        'ratio'] + importance_df['change'] * weights['change'] + (1 - importance_df['distance_to_change']) * weights[
                                      'distance']
    # sorting
