...
def create_quantile_feature_matrix(dataframe, feature, K, robust=False):
    ...
    baseline_exact_quantile = get_exact_quantile(percentile_function=percentile_function,
                                                 baseline_value=x_base[feature])
...
def nearest_quantile(dataframe, local_value, cat_features):
    ...
    if cat_features:
        ...
        quantile = dataframe.loc[
            dataframe['original'] == original_list[np.argmin(np.abs(original_list - local_value))], 'quantile']
...
def computeACME(model, features, numeric_df, cat_df, task, K, robust=False, class_to_analyze=None, score_function=None):
    ...
    for feature in features:
        ...
        if feature in cat_features:
            ...
            if cat_features != []:
                ...
                Z = pd.concat([Z_numeric.loc[Z_numeric.index.repeat(len(Z_cat))].reset_index(drop=True),
                               Z_cat.reset_index(drop=True)], axis=1)
            else:
                Z = Z_numeric
        else:
            ...
            if cat_features != []:
                Z_cat = cat_baseline
                Z = pd.concat([Z_cat.loc[Z_cat.index.repeat(len(Z_numeric))].reset_index(drop=True),
                               Z_numeric.reset_index(drop=True)], axis=1)
            else:
                Z = Z_numeric
        ...
        baseline_pred = compute_baseline_predictions(model=model, baseline=baseline, task=task,
                                                     score_function=score_function, class_to_analyze=class_to_analyze)
        predictions = compute_predictions(model=model, Z=Z, features=features, task=task, score_function=score_function,
        ...
        features_df['effect'] = (effects - np.mean(effects)) / np.sqrt(np.var(effects) + 0.0001) * (
    ...
    importance_table = table[['effect']].abs().reset_index().groupby('feature')[['effect']].mean().sort_values('effect',
                                                                                                               ascending=False).rename(
        columns={'effect': 'importance'})
