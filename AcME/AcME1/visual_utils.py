...
colors = ['#377eb8', '#ff7f00', '#4daf4a', '#f781bf', '#a65628', '#984ea3', '#999999', '#e41a1c', '#dede00',
          'midnightblue', 'limegreen', 'turquoise', '#858be3', 'wheat', 'lightslategray', 'lavender', 'rosyBrown']
patterns = ['', '/', '|', 'x', '-', '\\', '+', '.']


def feature_importance_distribution_barplot(features_occurence_table: pd.DataFrame, title: str = None,
                                            n_positions: int = None, threshold: float = 0,
                                            color_pattern_dict: dict = None):
    """
    ...
    for i, feature in enumerate(lengend_features):
        fig.add_trace(go.Bar(
            ...
            marker_color=feature_colors[feature] if color_pattern_dict is None else color_pattern_dict[feature][
                'color'],
            width=0.6,
            marker_pattern_shape=patterns[i % len(patterns)] if color_pattern_dict is None else
            color_pattern_dict[feature]['pattern'],
    ...
    fig.update_layout(
        barmode='stack',
        xaxis=dict(title='Ranking Position', tickmode='array', tickvals=features_occurence_table.index,
                   tickfont=dict(size=18), titlefont=dict(size=18)),
