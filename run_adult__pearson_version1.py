...
if __name__ == '__main__':
    with open("./dataset/adult/mapping_pearson.json", "r") as f:
        mapping = json.loads(f.read())

    raw_dataset = pd.read_csv(r"E:\yan\XAI\py\SFE-CF\cf\feature_select\dataset\adult\row_adult\adult_pearson.csv")
    ...
    dataset = pd.read_csv(r"E:\yan\XAI\py\SFE-CF\cf\feature_select\dataset\adult\processed_adult\processed_adult_pearson.csv")
    ...
    # load model
    model = tf.keras.models.load_model(r'E:\yan\XAI\py\SFE-CF\cf\feature_select\dataset\adult\pred_model\adult_pearson_DNN.h5')
    ...
    kd_tree = GeneratePopulationWithKmeans(black_model, raw_train_dataset, 500, mapping, n_clusters=50,
                                           random_state=0,
                                           pipeline_path=r"E:\yan\XAI\py\SFE-CF\cf\feature_select\dataset\adult\.pkl\adult_pearson_pipeline.pkl")
    ...
    df_original.to_csv(f"./dataset/adult/predict_samples_pearson.csv", index=False)
    df_cf.to_csv(f"./dataset/adult/counterfactual_samples_pearson.csv", index=False)
