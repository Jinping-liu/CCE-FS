...
def PrepareCompass(dataset_path):
    # Reading data from a csv file
    df = pd.read_csv(dataset_path)

    compas_dtypes = df.columns.to_series().groupby(df.dtypes).groups
    compas_dtypes = {k.name: v.tolist() for k, v in compas_dtypes.items()}

    continuous_features = compas_dtypes['int64']
    continuous_features = [feat for feat in continuous_features if feat != 'two_year_recid']
    ...
    joblib.dump(preprocessor, '../../dataset/compas/compas_pipeline.pkl')
    ...
    X.to_csv("../../dataset/compas/processed_compas.csv", index=False)
    ...
    with open("../../dataset/compas/mapping.json", "w") as f:
...
if __name__ == '__main__':
    PrepareCompass("../../dataset/compas/compass.csv")
