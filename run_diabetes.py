...
from cf.problem.diabetes import DiabetesProblemAVarConstVersion1
...
if __name__ == '__main__':
    with open(r"E:\yan\XAI\py\SFE-CF\dataset\diabetes\mapping_mic.json", "r") as f:
        mapping = json.loads(f.read())

    raw_dataset = pd.read_csv(r"E:\yan\XAI\py\SFE-CF\cf\feature_select\dataset\diabetes\row_diabetes\diabetes_mic.csv")

    target = raw_dataset['Outcome']
    ...
    dataset = pd.read_csv(r"E:\yan\XAI\py\SFE-CF\cf\feature_select\dataset\diabetes\processed_diabetes\processed_diabetes_mic.csv")
    ...
    # load model
    model = tf.keras.models.load_model(r'E:\yan\XAI\py\SFE-CF\cf\feature_select\dataset\diabetes\pred_model\diabetes_mic_DNN.h5')

    # eval feature importance
    black_model = BlackBox(model)
    black_model.evaluation_importance(train_dataset, train_target, target="Outcome", cat_features=False, is_show=False)
    prediction = black_model.predict(test_dataset.values)

    y_pred_original_dataset = raw_test_dataset.loc[(prediction == 1) & (test_target == 1)]
    ...
    k = 1
    kd_tree = GeneratePopulationWithKmeans(black_model, raw_train_dataset, 100, mapping, n_clusters=20,
                                           random_state=0,
                                           pipeline_path=r"E:\yan\XAI\py\SFE-CF\cf\feature_select\dataset\diabetes\.pkl\diabetes_mic_pipeline.pkl")
    ...
    for index, series_data in y_pred_original_dataset.iterrows():
        ...
        while True and repeat_count <= 2:
            ...
            raw_instance_target = query_instance['Outcome'].values[0]
            del query_instance['Outcome']

            population, query_instance_dummies = kd_tree.build_population(query_instance, 0, True, is_random)

            problem = DiabetesProblemAVarConstVersion1(
            ...
            try:
                ...
                cf_instance = find_best_solution_with_post_fix_original_dataset(
                    X, query_instance_dummies, black_model, kd_tree, 0, k=k,
                ...
                # todo 针对浮点数存在精度的问题,所以使用了该操作
                cf_mixed['DiabetesPedigreeFunction'] = query_instance['DiabetesPedigreeFunction']

                print(f"raw target: {raw_instance_target}，cf target: {0} \n")
    ...
    df_original.to_csv(r"E:\yan\XAI\py\SFE-CF\results\diabetes_mic\predict_samples_version_revised_1_100_IWFM_count.csv",
                       index=False)
    df_cf.to_csv(r"E:\yan\XAI\py\SFE-CF\results\diabetes_mic\counterfactual_samples_version_revised_1_100_IWFM_count.csv",
                 index=False)
