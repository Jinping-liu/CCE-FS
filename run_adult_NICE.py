...
# 加载数据
adult = pd.read_csv('cf/feature_select/dataset/adult/row_adult/adult.csv')
...
# 加载模型并验证输出形状
model = load_model('E:/yan/XAI/py/SFE-CF/cf/feature_select/dataset/adult/pred_model/adult_DNN.h5')
...
# 筛选target=0的原始样本
target_zero_indices = np.where(y == 0)[0]
...
# 保存文件
df_original.to_csv(os.path.join(save_dir, "adult/adult_NICE/predict_samples.csv"), index=False)
df_cf.to_csv(os.path.join(save_dir, "adult/adult_NICE/counterfactual_samples.csv"), index=False)
