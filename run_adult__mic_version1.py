...
from cf.problem.adult_problem import AdultProblemAVarConstVersion1
...
if __name__ == '__main__':
    with open("./dataset/adult/mapping_mic.json", "r") as f:
        mapping = json.loads(f.read())

    raw_dataset = pd.read_csv(r"E:\yan\XAI\py\SFE-CF\cf\feature_select\dataset\adult\row_adult\adult_mic.csv")
    ...
    dataset = pd.read_csv(r"E:\yan\XAI\py\SFE-CF\cf\feature_select\dataset\adult\processed_adult\processed_adult_mic.csv")
    ...
    # load model
    model = tf.keras.models.load_model(r'E:\yan\XAI\py\SFE-CF\cf\feature_select\dataset\adult\pred_model\adult_mic_DNN.h5')
    ...
    k = 1
    kd_tree = GeneratePopulationWithKmeans(black_model, raw_train_dataset, 100, mapping, n_clusters=50,
                                           random_state=0,
                                           pipeline_path=r"E:\yan\XAI\py\SFE-CF\cf\feature_select\dataset\adult\.pkl\adult_mic_pipeline.pkl")
    ...
    df_original.to_csv(f"./results/adult/adult_mic/predict_samples_mic(100,50).csv", index=False)
    df_cf.to_csv(f"./results/adult/adult_mic/counterfactual_samples_mic(100,50).csv", index=False)
